 ...
 */
function extractBitsReference(data: Uint8Array, bitOffset: number, bitLength: number): bigint {
...
describe('YulBitSlice', () => {
  describe('extractBits (reference model)', () => {
    ...
    it('throws on a zero bit length', () => {
      const data = new Uint8Array([0xab]);
      expect(() => extractBitsReference(data, 0, 0)).toThrow('BitLengthTooLarge');
    });

    it('throws when bit length exceeds 256', () => {
      const data = new Uint8Array([0xab]);
      expect(() => extractBitsReference(data, 0, 257)).toThrow('BitLengthTooLarge');
    });
  });
